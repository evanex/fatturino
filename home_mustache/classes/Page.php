...
class Page {
 ...
 private function load_data($data_dir) {
 	...
 	$include_file = $data_dir."/php/".$this->name.".inc.php";

 	if(file_exists($include_file)){
 		require_once($include_file);
 		$page_data_manager = new data_manager();
 	if(!is_array($page_data_manager->data)){return true;}
 	foreach($page_data_manager->data as $var_name => $new_data){
 	 	$this->vars[$var_name] = $new_data;
 	 }
 	 }
   ...
   //  echo $partial_data_folder;
	...
	foreach($data_files as $var_name => $var_value) {
			...
			//for example change sidebar.navList to sidebar_navList , because sidebar is not an object
		  	...
		  	$this->vars['user_email']=$_SESSION['user_email'];
